...
var app = angular.module("app", ["ngAnimate", "ngResource", "ui.router", "ui.bootstrap", "ui.validate", "pascalprecht.translate", "messager", "toaster", "hc.util", "hc.ui", "hc.common", "ui.codemirror"]);
app.config(["$controllerProvider", "$compileProvider", "$filterProvider", "$provide", function (t, e, n, r) {
    app.controller = t.register, app.directive = e.directive, app.filter = n.register, app.factory = r.factory, app.service = r.service, app.constant = r.constant, app.value = r.value
}]), window.require && function (t) {
    "function" == typeof define && define.amd ? (require.config({
        paths: {app: "/asset/src/app"},
        urlArgs: "_dc=" + (new Date).getTime()
    }), define(["app/app.config", "app/services/fileSystem", "app/services/webUploader", "app/services/dropbox", "app/services/fileOperation", "app/controllers/app", "app/controllers/directory", "app/controllers/document", "app/controllers/authorize", "app/controllers/favorites", "app/controllers/recents", "app/controllers/recycle", "app/controllers/share", "app/controllers/changePassword", "app/controllers/selectUser", "app/controllers/profile", "app/controllers/dropbox", "app/controllers/messages", "app/controllers/search"], t)) : t()
}(function () {
}), app.config(["$translateProvider", function (t) {
    t.useStaticFilesLoader({prefix: "asset/l10n/", suffix: ".json"}), t.preferredLanguage("zh_CN")
}]).factory("responseObserver", ["$q", "$window", function (t, e) {
    return {
        responseError: function (n) {
            switch (n.status) {
                case 401:
                    e.location.href = "/login"
            }
            return t.reject(n)
        }
    }
}]).config(["$httpProvider", function (t) {
    t.interceptors.push("responseObserver");
    var e = {"Content-Type": "application/json", "X-Requested-With": "XMLHttpRequest"};
    e[crsf.headerName] = crsf.token, t.defaults.headers.post = e, t.defaults.headers.get = {"X-Requested-With": "XMLHttpRequest"}
}]), app.run(["$rootScope", "$state", "$stateParams", "$http", "Messager", function (t, e, n, r, s) {
    t.$state = e, t.$stateParams = n
}]).config(["$stateProvider", "$urlRouterProvider", "$locationProvider", function (t, e, n) {
    n.html5Mode(!0), e.otherwise("/index/directory/enterprise"), e.when("/index/document/{document}", ["$state", "$match", "$rootScope", function (t, e, n) {
        n.fid = e.document
    }]), t.state("favorites", {
        url: "/index/favorites",
    ...
    }).state("file", {url: "/index/directory/{directory}/{file}"}).state("changePassword", {
...
}]), app.controller("AppCtrl", ["$rootScope", "$scope", "toaster", "FileSystem", "Messager", "$http", "$modal", "$location", "$state", "environment", "WebUploader", "Dropbox", "FileOperation", function (t, e, n, r, s, i, o, l, a, c, d, p, u) {
    e.ie9 = /msie 9/.test(window.navigator.userAgent.toLowerCase()), e.supportWebkitDirectory = "webkitdirectory" in HTMLInputElement.prototype, e.showQueue = !1, t.toaster = n, t.files = [], t.permissions = c.permissions, t.space = c.space, t.resourceServerUrl = "/", t.staticResourceServerUrl = "/", t.openDetail = !0, t.appSetting = c.appSetting, t.toggleDetail = function () {
        t.openDetail = !t.openDetail, t.$apply()
    }, d.init(t), p.init(e), u.init(e), t.user = c.user, t.organization = c.user.organization, t.space = c.space, t.space.quotaPrecent = t.space.used / t.space.quota, t.mounts = c.mounts, t.mountAliases = {};
    for (var g in e.mounts) {
        var m = e.mounts[g];
        t.mountAliases[m.id] = m.alias
    }
    if (e.$on("$stateChangeStart", function (n, r, s, i) {
        t.module = r.name, !0 === e.user.changePassword && "changePassword" !== r.name && (n.preventDefault(), a.go("changePassword"))
    }), t.mediaType = {
    ...
    }, e.onSearchKeyDown = function (t) {
        13 === t.keyCode && e.search()
    }, e.search = function (t) {
        e.keyword && a.go("search", {keyword: e.keyword})
    }, e.authorize = function (t, n) {
        if (e.isAdmin() || "my" === e.mountAlias) return !0;
        if ("enterprise" === e.router) return "download" === t;
        if (e.current) return !(s = e.current.permission) || 0 === s.length || s.indexOf(t) >= 0;
        var r = n ? [n] : e.selection;
        if (!r) return !1;
        var s = e.permissions, i = r.length;
        if (!s) return !0;
        for (var o = 0; o < i; o++) {
            var l = s[r[o].id];
            if (l && l.indexOf(t) < 0) return !1
        }
        return !0
    }, t.getMountAlias = function (e) {
        return t.mountAliases[e]
    }, t.setCurrent = function (e) {
        e ? (t.mountAlias = t.mountAliases[e.mount], t.mount = t.mounts[t.mountAlias], t.router = e.id, t.driectoryName = e.name) : "my" === t.mountAlias ? t.driectoryName = "个人文档" : t.driectoryName = "企业文档", t.current = e
    }, t.isAdmin = function () {
        return ["SYS_ADMIN", "DOC_ADMIN"].indexOf(t.user.role) >= 0
    }, e.$on("$locationChangeSuccess", function (e, n, r) {
        t.fid ? t.openFile() : t.closeFile()
    }), t.getExt = function (t) {
        if (t) {
            var e = t.name;
            if (e) return e.substring(e.lastIndexOf(".") + 1, e.length)
        }
    }, t.getIcon = function (e) {
        var n;
        if (!e) return null;
        if ("directory" === e.type) n = Util.isPrivate(e) ? "asset/img/private_folder.png" : "asset/img/public_folder.png"; else {
            var r = t.getExt(e);
            if (!r) return;
            r = r.toLowerCase();
            var s = t.mediaType[r];
            n = s ? s.icon : "asset/img/file/file_unknown.png"
        }
        return n
        ...
        i.post("/user/set", {
            name: e.user.name,
            mode: e.user.mode,
            sortField: e.user.sortField,
            sortDesc: e.user.sortDesc
        })
    }, e.sortField = e.user.sortField || "name", e.sortFields = ["type", e.sortField], e.sortDesc = e.user.sortDesc || !1, e.mode = e.user.mode || "list", e.showTags = !0, e.switchShowTag = function () {
        e.showTags = !e.showTags
    }, e.switchMode = function (t) {
        e.mode = t || ("list" === e.mode ? "block" : "list"), e.user.mode = e.mode
    }, e.changeSort = function (t) {
        e.sortField === t ? e.sortDesc = !e.sortDesc : (e.sortField = t, e.sortDesc = !0), e.sortFields = ["type", t], e.user.sortField = e.sortField, e.user.sortDesc = e.sortDesc
    }, e.changeDesc = function () {
        e.sortDesc = !e.sortDesc, e.user.sortDesc = e.sortDesc
    }, t.open = function (e, n) {
        "directory" === e.type ? a.go("directory", {directory: e.id}) : (t.fid = e.id, l.path("/index/document/" + e.id), n && n.stopPropagation())
    }, c.notice) {
        var f = c.notice;
        s.alert(f.title, f.content)
    }
    t.started = !0
}]), angular.element(document).ready(function () {
    function t() {
        angular.bootstrap(document, ["app"]), window.clearInterval(window.preloadInterval)
    }
    
    $.ajax({url: "/environment", method: "get"}).success(function (e) {
        e.success ? (delete e.success, app.value("environment", e.data), t()) : alert("加载用户信息失败，请重新刷新页面再试")
    }).error(function () {
        alert("加载用户信息失败，请重新刷新页面再试")
    })
}), app.controller("AuthorizePublicCtrl", ["$rootScope", "$scope", "$http", "$modal", "$modalInstance", "data", function (t, e, n, r, s, i) {
    e.permission = i.permission;
    var o = t.toaster.wait("正在加载权限...");
    n.get("/conf/permission").success(function (t) {
        t.success ? (e.permissions = t.permissions, o.doSuccess("权限已加载")) : o.doError("加载权限列表失败:" + t.message)
    }).error(function (t, e) {
        o.doError("加载权限列表失败:" + (t.mesage || e))
    }), e.ok = function () {
        var r = {permission: e.permission, id: i.id};
        e.saving = !0;
        var o = t.toaster.wait("正在设置权限");
        n.post("/fs/permission/set", r).success(function (t) {
            e.saving = !1, t.success ? (o.doSuccess("权限已设置"), s.close()) : o.doError("权限设置失败:" + t.message)
        }).error(function (t, n) {
            e.saving = !1, o.doError("权限设置失败:" + (t.message || n))
        })
    }, e.close = function () {
        s.dismiss("cancel")
    }
}]), app.controller("AuthorizeCtrl", ["$rootScope", "$scope", "$http", "Messager", "$modal", "$modalInstance", "id", function (t, e, n, r, s, i, o) {
    e.permissionMap = {};
    for (var l = 0; l < e.permissions.length; l++) e.permissionMap[e.permissions[l].id] = e.permissions[l].name;
    e.select = function (t, n) {
        n.stopPropagation(), e.current !== t ? e.current = t : e.current = null
    }, e.condition = {}, e.changeConditionPermission = function (t) {
        e.condition.permission = t
    }, e.load = function () {
        e.loading = !0, n.get("/fs/authorize/list", {params: {documentId: o}}).success(function (t, n, r, s) {
            e.loading = !1, t.success ? e.authorizes = t.data : (e.toaster.pop("error", t.message), e.error = t.message)
        }).error(function (t, n) {
            e.loading = !1, e.error = t.message, e.toaster.pop("error", "加载失败：" + n)
        })
    }, e.count = function (t) {
        if (!e.authorize || 0 === e.authorize.length) return !1;
        var n = 0;
        return e.authorize.forEach(function (e) {
            e.type === t && n++
        }), n > 0
    }, e.add = function () {
        s.open({
        ...
        function s() {
            var r = e.toaster.wait("正在更新权限...");
            if (!0 === t.inherited) {
                var s = {documentId: o, permission: e.permissionId};
                1 === e.current.ownerType ? s.userIds = [e.current.owner] : s.departmentIds = [e.current.owner], n.post("/fs/authorize/add", s).success(function (t) {
                    t.success ? (r.doSuccess("权限已更新"), e.load()) : r.doError("权限更新失败:" + t.message)
                }).error(function (t, e) {
                    r.doError("权限更新失败:" + (t.message || e))
                })
            } else n.post("/fs/authorize/update", {id: t.id, permissionId: e.permissionId}).success(function (t) {
                t.success ? (e.load(), r.doSuccess("权限已更新")) : r.doError("权限更新失败:" + t.message)
            }).error(function (t, e) {
                r.doError("权限更新失败:" + (t.message || e))
            })
        }
        
        e.permissionId !== t.permissionId && r.confirm("提示", "确定要修改权限?").then(function () {
            s()
        })
    }, e.remove = function (t, s) {
        function i(t) {
            var r = e.toaster.wait("正在删除权限...");
            n.post("/fs/authorize/remove/" + t.id, {}).success(function (t) {
                t.success ? (e.load(), e.current = null, r.doSuccess("权限已删除")) : r.doError("删除权限失败:" + t.message)
            }).error(function (t, e) {
                r.doError("权限删除失败:" + (result.message || e))
            })
        }
        
        s.stopPropagation(), r.confirm("提示", "确定要删除?").then(function () {
            i(t)
        })
    }, e.ok = function () {
        i.close(result)
...
}]), app.controller("AuthorizeAddCtrl", ["$rootScope", "$scope", "$http", "$modal", "$modalInstance", "permissions", "id", function (t, e, n, r, s, i, o) {
    e.loading = !1, e.tree = {}, e.permissions = i, e.onTreeInit = function () {
        var t = e.tree.getRoot();
        e.tree.select(t)
    }, e.clear = function () {
        e.searching = !1, e.condition = null
    }, e.selectedUsers = [], e.selectedUserIds = [], e.selectedDepartments = [], e.selectedDepartmentIds = [], e.addDepartment = function (t) {
        e.selectedDepartmentIds.indexOf(t.id) >= 0 || (e.selectedDepartments.push(t), e.selectedDepartmentIds.push(t.id))
    }, e.removeDepartment = function (t) {
        e.selectedDepartmentIds.indexOf(t.id) < 0 || (e.selectedDepartments.splice(e.selectedDepartments.indexOf(t), 1), e.selectedDepartmentIds.splice(e.selectedDepartmentIds.indexOf(t.id), 1))
    }, e.addUser = function (t) {
        e.selectedUserIds.indexOf(t.id) >= 0 || (e.selectedUsers.push(t), e.selectedUserIds.push(t.id))
    }, e.removeUser = function (t) {
        e.selectedUserIds.indexOf(t.id) < 0 || (e.selectedUsers.splice(e.selectedUsers.indexOf(t), 1), e.selectedUserIds.splice(e.selectedUserIds.indexOf(t.id), 1))
    }, e.search = function () {
        e.condition && (e.searching = !0, e.loading = !0, n.get("/member/search", {params: {condition: e.condition}}).success(function (t, n, r, s) {
            e.loading = !1, t.success ? e.searchResult = t.data : e.toaster.pop("error", "提示", t.message)
        }).error(function (t, n) {
            e.loading = !1, e.toaster.pop("error", "提示", "加载失败：" + n)
        }))
    }, e.ok = function () {
        var t = {
            documentId: o,
            userIds: e.selectedUserIds,
            departmentIds: e.selectedDepartmentIds,
            permission: e.permission
        }, r = e.toaster.wait("正在添加权限...");
        e.saving = !0, n.post("/fs/authorize/add", t).success(function (t) {
            e.saving = !1, t.success ? (r.doSuccess("权限已添加"), s.close()) : r.doError("添加权限失败：" + t.message)
        }).error(function (t, n) {
            e.saving = !1, r.doError("添加权限失败：" + (t.message || n))
        })
    }, e.loadMember = function (t) {
        e.loading = !0, n.get("/member/list", {params: {departmentId: t.id}}).success(function (t, n, r, s) {
            e.loading = !1, t.success ? (e.users = t.data.users, e.departments = t.data.departments) : e.toaster.pop("error", t.message)
        }).error(function (t, n) {
            e.loading = !1, e.toaster.pop("error", "加载失败：" + n)
        })
    }, e.close = function () {
        s.dismiss("cancel")
    }
}]), app.controller("ChangePasswordCtrl", ["$scope", "$http", function (t, e) {
    t.check = function () {
        var e = 0;
        if (t.newPassword.length < 1) return e;
        /\d/.test(t.newPassword) && e++, /[a-z]/.test(t.newPassword) && e++, /[A-Z]/.test(t.newPassword) && e++, /\W/.test(t.newPassword) && e++, t.strength = e
    }, t.submit = function () {
        e.post("/user/change/password", {
            oldPassword: t.oldPassword,
            newPassword: t.newPassword,
            confirmPassword: t.confirmPassword
        }).success(function (e) {
            e.success ? (t.newPassword = null, t.oldPassword = null, t.confirmPassword = null, t.success = !0, t.cause = null, t.user.changePassword = !1) : (t.cause = e.message, t.success = !1)
        }).error(function (e, n) {
            t.error = e.message || n, t.success = !1
        })
    }
}]), app.controller("DirectoryCtrl", ["$rootScope", "$scope", "$http", "$state", "$location", "$modal", "$stateParams", "Messager", "FileSystem", function (t, e, n, r, s, i, o, l, a) {
    function c(t) {
        angular.element("#directoryOperationContextMenu").hide(), angular.element("#containerContextMenu").hide(), angular.element("#permissionContextMenu").hide()
    }
    
    function d(t) {
        116 === t.keyCode && (t.preventDefault(), e.reload())
    }
    
    e.ie9 && e.webUploader.addButton({
        id: "#filePicker",
        innerHTML: '<i class="glyphicon glyphicon-cloud-upload"></i> 上传文件'
    }), t.module = "directory";
    var p = o.directory, u = t.mounts[p];
    u && (t.mount = u, t.mountAlias = p, p = null), e.$on("$viewContentLoaded", function () {
        e.load(p)
    }), e.reload = function () {
        e.current ? e.load(e.current.id) : e.load()
    }, e.load = function (n) {
        e.loading = !0, e.reset();
        var r = e.mount ? e.mount.id : null;
        a.load(r, n).then(function (n) {
            e.files = n.files, e.path = n.path, e.activities = n.activities, e.authorizes = n.authorizes, n.current && (n.current.permissions = n.permissions), t.setCurrent(n.current), e.loading = !1
        }, function (t) {
            e.error = t, e.toaster.pop("error", "加载文件失败", t), e.loading = !1
        })
    }, e.checkPermission = function (n) {
        if ("my" === e.mountAlias) return !0;
        if ("create" === n && !t.current && "enterprise" === e.mountAlias) return !0;
        if ("upload" === n && !t.current && "enterprise" === e.mountAlias) return !1;
        if (e.current) {
            if (e.current.creationBy.id === t.user.id) return !0;
            if (e.current.permissions) return e.current.permissions.indexOf(n) >= 0
        }
        return !1
    }, e.loadAuthorizes = function () {
        n.get("/fs/authorize/list", {params: {documentId: e.current.id}}).success(function (t, n, r, s) {
            e.loading = !1, t.success ? e.authorizes = t.data : e.toaster.pop("error", t.message)
        }).error(function (t, n) {
            e.loading = !1, e.toaster.pop("error", "加载失败：" + n)
        })
    }, e.$on("$fileUploaded", function (t, n) {
        e.current ? (n.parent = e.current.id) && e.reload() : n.mount === e.mount.id && e.reload()
    }), e.$on("$currentFolderReload", function (t, n) {
        e.reload()
    }), e.addFavorite = function (e, n) {
        n.stopPropagation(), a.addFavorite(e).then(function (n) {
            t.toaster.success({title: "已收藏"}), e.favoriteId = n.data
        }, function () {
            t.toaster.error({title: "收藏失败"})
        })
    }, e.removeFavorite = function (e, n) {
        n.stopPropagation(), a.removeFavorite(e).then(function (n) {
            t.toaster.success({title: "收藏已取消"}), e.favoriteId = null
        }, function () {
            t.toaster.error({title: "取消收藏失败"})
        })
    }, e.canShowAuthorizer = function (t) {
        return null != t && (a.isDirectory(t) && "my" !== e.mountAlias)
    }, e.canEditPermission = function (e) {
        return null != e && e.creationBy.id === t.user.id
    }, e.isNonLocked = function (t) {
        return 0 === e.lockedSelectionCount
    }, e.authorizeUnlock = function (t) {
        if (!1 === e.hasPermission("lock")) return !1;
        for (var n = e.selection, r = 0; r < n.length; r++) if (!0 !== (t = n[r]).locked || t.lockedBy && t.lockedBy.id === e.user.id) return !1;
        return !0
    }, e.isSelectAll = !1, e.selection = [], e.lockedSelectionCount = 0, e.selectAll = function () {
        e.isSelectAll = !e.isSelectAll;
        for (var t = e.files.length, n = 0; n < t; n++) e.select(e.files[n], e.isSelectAll)
    }, e.addSelection = function (t) {
        e.selection.push(t), t.selected = !0, !0 === t.locked && e.lockedSelectionCount++, e.selection.length === e.files.length && (e.isSelectAll = !0)
    }, e.removeSelection = function (t) {
        e.selection.splice(e.selection.indexOf(t), 1), t.selected = !1, !0 === t.locked && e.lockedSelectionCount++, e.isSelectAll = !1
    }, e.clearSelection = function () {
        e.selection.forEach(function (t) {
            t.selected = !1
        }), e.selection = [], e.isSelectAll = !1
    }, e.select = function (t, n, r) {
        if (r && "block" === e.mode && !r.ctrlKey && e.clearSelection(), r && r.stopPropagation(), !t.selected || n !== t.selected) return $(".app-ctrl").scrollTop(0), !1 === (n = n || !t.selected || !t.selected) ? e.removeSelection(t) : e.addSelection(t)
    }, e.removeAuthorize = function (t, r) {
        function s(t) {
            var r = e.toaster.wait("正在删除..."), s = n.post("/fs/authorize/remove/" + t.id, {});
            s.success(function (t) {
                t.success ? (e.loadAuthorizes(), r.doSuccess("已删除")) : r.doError("删除失败:" + t.message)
            }), s.error(function (t, e) {
                r.doError("删除失败:" + (t.message || e))
            })
        }
        
        r.stopPropagation(), l.confirm("提示", "确定要删除?").then(function () {
            s(t)
        })
    }, e.addAuthorize = function () {
        i.open({
            templateUrl: "asset/tpl/app/authorize_add.html",
            controller: "AuthorizeAddCtrl",
            size: "lg",
    ...
    }, e.updatePermission = function (t) {
        function r() {
            var r = e.toaster.wait("正在修改权限...");
            if (!0 === e.selectedAuthorize.inherited) {
                var s = {documentId: e.current.id, permission: t.id};
                1 === e.selectedAuthorize.ownerType ? s.userIds = [e.selectedAuthorize.owner] : s.departmentIds = [e.selectedAuthorize.owner], n.post("/fs/authorize/add", s).success(function (t) {
                    t.success ? (r.doSuccess("权限已更新"), e.loadAuthorizes()) : r.doError("权限更新失败:" + t.message)
                }).error(function (t, e) {
                    r.doError("权限更新失败:" + (t.message || e))
                })
            } else n.post("/fs/authorize/update", {
                id: e.selectedAuthorize.id,
                permissionId: t.id
            }).success(function (t) {
                t.success ? (r.doSuccess("权限已更新"), e.loadAuthorizes()) : r.doError("权限更新失败:" + t.message)
            }).error(function (t, e) {
        ...
        t.id !== e.selectedAuthorize.permissionId && l.confirm("提示", "确定要修改协作者的权限?").then(function () {
            r()
        })
    }, e.onContainerClick = function () {
        "block" === e.mode && e.clearSelection()
    }, e.filterTag = function (t, n) {
        e.condition = t, n.stopPropagation()
    }, e.download = function (n) {
        n || (n = e.selection[0]), t.download(n)
    }, e.remove = function (n) {
        n || 0 === (n = e.selection).length && l.alert("提示", "请选择要删除的文件"), t.remove(n, function () {
            e.reload()
        })
    }, e.lock = function (n) {
        n || (n = e.selection[0]) || l.alert("提示", "请选择要锁定的文件"), t.lock(n, function () {
            e.reload()
        })
    }, e.unlock = function (n) {
        n || (n = e.selection[0]) || l.alert("提示", "请选择要解锁的文件"), t.unlock(n, function () {
            e.reload()
        })
    }, e.rename = function (n) {
        n || (n = e.selection[0]), n || l.alert("提示", "请选择要重命名的文件"), t.rename(n, function (t, e) {
            n.name = e
        })
    }, e.showAuthorizer = function (t) {
        t = t || e.selection[0];
        var n;
        n = {
        ...
        }, i.open(n).result.then(function (t) {
            e.reload()
        })
    }, e.move = function () {
        t.move(e.selection, e.mount, function (t) {
            e.reload()
        })
    }, e.copy = function () {
        t.copy(e.selection, e.mount, function (t) {
            e.reload()
        })
    }, e.updateDesc = function () {
        t.updateDescription(e.current, function () {
        }, function () {
        })
    }, e.createDirectory = function () {
        l.prompt("新建文件夹", "", "").then(function (n) {
            var r = e.toaster.wait({title: "正在新建文件夹"}), s = {mount: t.mount.id, name: n};
            e.current && (s.parent = e.current.id), a.createDirectory(s).then(function () {
                r.doSuccess({type: "success", title: "新建文件夹成功"}), e.reload()
            }, function (t) {
                r.doError({type: "error", title: "新建文件夹失败：" + t})
            })
        })
    }, e.showContextMenu = function (t, n) {
        var r = angular.element("#directoryOperationContextMenu"), s = angular.element(window.document);
        e.selected = t, e.select(t, !0, n);
        var i = n.pageY, o = n.pageX;
        i + r.height() > s.height() && (i -= r.height()), o + r.width() > s.width() && (o -= r.width()), r.css({
            top: i,
            left: o
        }), r.show(), n.stopPropagation()
    }, e.showDirectoryContextMenu = function (t, n) {
        var r = angular.element("#directoryOperationContextMenu"), s = angular.element(window.document);
        e.selected = t;
        var i = n.pageY, o = n.pageX;
        i + r.height() > s.height() && (i -= r.height()), o + r.width() > s.width() && (o -= r.width()), r.css({
    ...
    }, e.showPermissionContextMenu = function (t, n) {
        var r = angular.element("#permissionContextMenu"), s = angular.element(window.document);
        e.selectedAuthorize = t;
        var i = n.pageY, o = n.pageX;
        i + r.height() > s.height() && (i -= r.height()), o + r.width() > s.width() && (o -= r.width()), r.css({
            top: i,
            left: o
        }), r.show(), n.stopPropagation()
    }, e.showContainerContextMenu = function (t) {
        var e = angular.element("#containerContextMenu"), n = angular.element(window.document), r = t.pageY,
            s = t.pageX;
        r + e.height() > n.height() && (r -= e.height()), s + e.width() > n.width() && (s -= e.width()), e.css({
            top: r,
            left: s
        }), e.show(), t.stopPropagation()
    }, e.reset = function () {
        e.selection = [], e.isSelectAll = !1, e.lockedSelectionCount = !1
    }, $(document).on("keydown", d), $(window).on("click", c), e.$on("$destroy", function () {
        $(window).unbind("click", c), $(document).unbind("keydown", d), t.setCurrent(null)
    })
}]), app.controller("DirectoryFormCtrl", ["$rootScope", "$scope", "$modalInstance", function (t, e, n) {
    e.ok = function () {
        var t = {name: e.name, type: e.type};
        n.close(t)
    }, e.allowPrivate = function () {
        return "enterprise" === t.mountAlias && e.isAdmin() && !t.current
    }, e.close = function () {
        n.dismiss("cancel")
    }
}]), app.controller("FileDetailCtrl", ["$rootScope", "$scope", "$http", "FileSystem", "Messager", "$location", "$modal", "$modalInstance", "id", function (t, e, n, r, s, i, o, l, a) {
    var c = document.body,
        d = c.requestFullScreen || c.webkitRequestFullScreen || c.mozRequestFullScreen || c.msRequestFullScreen;
    e.supportFullScreen = d, e.fullScreen = function () {
        var t = document.getElementById("file-preview-container");
        (t.requestFullScreen || t.webkitRequestFullScreen || t.mozRequestFullScreen || t.msRequestFullScreen).call(t), e.hiddenDetail = !0
    }, e.deg = 0, e.rotate = function () {
        e.deg += 90, 360 === e.deg && (e.deg = 0)
    }, e.addReview = function (t) {
        var r = e.toaster.wait("正在发表评论");
        n.post("/review/add", {content: t, documentId: e.file.id}).success(function (t) {
            if (t.success) {
                r.doSuccess("评论发表成功");
                var n = t.data;
                n.name = e.user.name, n.avatar = e.user.avatar, e.reviews.push(n)
            } else r.doError("发表评论失败: " + t.message)
        }).error(function (t, e) {
            r.doError("发表评论失败: " + (t.message || e))
        })
    }, e.removeReview = function (t) {
        s.confirm("提示", "确定要删除评论?").then(function () {
            var r = e.toaster.wait("正在删除评论");
            n.post("/review/delete", {id: t.id}).success(function (n) {
                n.success ? (r.doSuccess("评论已删除"), e.reviews.splice(e.reviews.indexOf(t), 1)) : r.doError("删除评论失败: " + n.message)
            }).error(function (t, e) {
                r.doError("删除评论失败: " + (t.message || e))
            })
        })
    }, e.addFavorite = function () {
        r.addFavorite(e.file).then(function () {
            t.toaster.success({title: "已收藏"}), e.favorited = !0
        }, function () {
            t.toaster.error({title: "收藏失败"})
        })
    }, e.removeFavorite = function () {
        r.removeFavorite(e.file).then(function () {
            t.toaster.success({title: "收藏已取消"}), e.favorited = !1
        }, function () {
            t.toaster.error({title: "取消收藏失败"})
        })
    }, e.remove = function () {
        t.remove(e.file, function () {
            e.close(!0)
        })
    }, e.rename = function () {
        t.rename(e.file, function (t, n) {
            e.file.name = n
        })
    }, e.move = function () {
        t.move([e.file], e.file.mount, function (t) {
            e.close(!0)
        })
    }, e.lock = function () {
        t.lock(e.file, function (t) {
            e.load(a)
        })
    }, e.unlock = function () {
        t.unlock(e.file, function (t) {
            e.load(a)
        })
    }, e.copy = function () {
        t.copy([e.file])
    }, e.download = function (n) {
        n || (n = e.file), t.download(n)
    }, e.load = function (r) {
        e.loading = !0, n.get("/fs/document/" + r).success(function (n) {
            e.loading = !1, n.success ? (e.file = n.data.document, e.favorited = null != n.data.document.favoriteId, e.permissions = n.data.permissions, e.activities = n.data.activities, e.reviews = n.data.reviews, e.token = n.data.token, e.setCurrent(e.file), t.mount || (t.mountAlias = t.mountAliases[e.file.mount], t.mount = t.mounts[t.mountAlias])) : e.error = n.message || "加载失败"
        }).error(function (t, n) {
            e.loading = !1, e.error = n
        })
    }, e.setVersion = function (r) {
        s.confirm("提示", "确定将该版本设置为最新版本?").then(function () {
            var s = t.toaster.wait("正在设置新版本...");
            n.post("/fs/version/set", {id: r.id}).success(function (t) {
                t.success ? (s.doSuccess("设置最新版本成功"), e.load(t.file.id)) : s.doError("设置最新版本失败:" + t.message)
            }).error(function (t, e) {
                s.doError("设置最新版本失败:" + (t.message || e))
            })
        })
    }, e.deleteVersion = function (r) {
        s.confirm("提示", "版本删除之后不可恢复，确定删除该版本?").then(function () {
            var s = t.toaster.wait("正在删除...");
            n.post("/fs/version/delete/" + r.id).success(function (t) {
                t.success ? (s.doSuccess("删除版本成功"), e.load(e.file.id)) : s.doError("删除版本失败:" + t.message)
            }).error(function (t, e) {
                s.doError("删除版本失败:" + (t.message || e))
            })
        })
    }, e.setCurrent = function (r) {
        e.current = r, e.current.media = e.getMediaType(e.current), e.current.icon = t.getIcon(e.current), "code" === e.current.media && (e.cmOption = {
            lineNumbers: !0,
            indentWithTabs: !0,
            mode: e.getExt(r),
            readOnly: !0,
            onLoad: function (t) {
                t.setOption("css", "height:100%")
            }
        }, n.get("/fs/raw?token=" + e.token).success(function (t) {
            e.raw = t
        }))
    }, e.checkPermission = function (t) {
        return "my" === e.mountAlias || (!(e.permissions && e.permissions.length > 0) || e.permissions.indexOf(t) >= 0)
    }, e.getMediaType = function (t) {
        var n = e.getExt(t);
        if (n) {
            var r = e.mediaType[n.toLowerCase()];
            return r ? r.type : "unknow"
        }
    }, e.geImageSrc = function (t) {
        return "/fs/raw?token=" + e.token
    }, e.getPdfViewerUrl = function (t) {
        return "asset/pdfjs/viewer.html?token=" + e.token
    }, e.getPreviewUrl = function (t) {
        return "/fs/office?id=" + e.current.id
    }, e.close = function (n) {
        l.close(n);
        var r = "/index/";
        if (t.module) switch (t.module) {
            case"search":
                r += "search/" + t.keyword;
                break;
            case"directory":
            case void 0:
                r += "directory/" + (-1 !== e.file.parent ? e.file.parent : t.getMountAlias(e.file.mount));
                break;
            default:
                r += t.module
        } else r += "directory/enterprise";
        i.path(r)
    }, e.onContainerClick = function () {
        angular.element("#documentOperationContextMenu").hide()
    }, e.load(a), e.showDocumentContextMenu = function (t, n) {
        var r = angular.element("#documentOperationContextMenu"), s = angular.element(window.document);
        e.selected = t;
        var i = n.pageY, o = n.pageX;
        i + r.height() > s.height() && (i -= r.height()), o + r.width() > s.width() && (o -= r.width()), r.css({
            top: i,
...
}]), app.controller("FileMoveCtrl", ["$rootScope", "$scope", "Messager", "$modalInstance", "data", function (t, e, n, r, s) {
    Util.Array.copyProperty(s.files, "id", function (t) {
        return "directory" === t.type
    });
    e.isRepeat = !1, e.ok = function () {
        n.confirm("提示", "是否确定要移动?").then(function () {
            r.close(e.target)
        })
    }, e.setParam = function (t, e) {
        e.mount = t.mount, e.excludes
    }, e.setTarget = function (t) {
        var n = s.files[0].parent, r = s.files[0].mount;
        if (n && n === t.id ? e.isRepeat = !0 : n || t.id || r !== t.mount ? e.isRepeat = !1 : e.isRepeat = !0, !0 === e.isRepeat) return e.target = null, void (e.paths = null);
        var i = [];
        for (e.target = t, i.push(t.name); t.$parent && !t.$parent.$root;) t = t.$parent, i.push(t.name);
        i.reverse(), e.paths = i.join(" / ")
    }, e.updateDesc = function () {
        t.updateDescription(e.current, function () {
        }, function () {
        })
...
}]), app.controller("FileCopyCtrl", ["$rootScope", "$scope", "Messager", "$modalInstance", "data", function (t, e, n, r, s) {
    e.ok = function () {
        n.confirm("提示", "是否确定要复制?").then(function () {
            r.close(e.target)
        })
    }, e.setParam = function (t, e) {
        e.mount = t.mount
        ...
        var n = [];
        for (e.target = t, n.push(t.name); t.$parent && !t.$parent.$root;) t = t.$parent, n.push(t.name);
        n.reverse(), e.paths = n.join(" / ")
    }, e.close = function () {
        r.dismiss()
    }
}]), app.controller("FileTagCtrl", ["$scope", "$http", "$modalInstance", "Messager", "FileSystem", "file", function (t, e, n, r, s, i) {
    t.tags = [], t.file = i, i.tags && (t.tags = i.tags.split(",")), t.remove = function (e) {
        t.tags.splice(t.tags.indexOf(e), 1), i.tags = t.tags.join(","), t.save()
    }, t.add = function () {
        t.tags.indexOf(t.tag) >= 0 ? t.toaster.pop("error", "存在相同的标签") : (t.tags.push(t.tag), t.tag = null, i.tags = t.tags.join(","), t.save())
    }, t.save = function () {
        s.setTag(t.file).then(function (t) {
        })
    }, t.cancel = function () {
        n.dismiss("cancel")
    }
}]), app.controller("FileSendCtrl", ["$scope", "$http", "files", "$modal", "$modalInstance", "Messager", function (t, e, n, r, s, i) {
    t.files = n, t.remove = function (e) {
        t.files.splice(t.files.indexOf(e), 1), 0 === t.files.length && s.dismiss("cancel")
    }, t.users = [], t.userIds = {}, t.addReceiver = function () {
        r.open({
            ...
            e.forEach(function (e) {
                t.userIds[e.id] || (t.users.push(e), t.userIds[e.id] = e)
            })
        })
    }, t.removeReceiver = function (e) {
        t.users.splice(t.users.indexOf(e), 1), delete t.userIds[e.id]
    }, t.ok = function () {
        var n = t.toaster.wait("正在发送");
        t.sending = !0;
        var r = Util.Array.copyProperty(t.files, "id"), i = Util.Array.copyProperty(t.users, "id");
        e.post("/message/send", {files: r, receivers: i, content: t.content}).success(function (e) {
            t.sending = !1, e.success ? (n.doSuccess("发送成功"), s.close()) : n.doError("发送失败:" + e.message)
        }).error(function (t, e) {
            n.doError("发送失败:" + (t.message || e))
        })
    }, t.cancel = function () {
        s.dismiss("cancel")
    }
}]), app.controller("DropboxCtrl", ["$scope", "$http", "data", "$modalInstance", "Messager", function (t, e, n, r, s) {
    t.files = [];
    for (var i = 0; i < n.files.length; i++) console.log(n.files), t.files.push(n.files[i]);
    t.treeConfig = {
        "load-url": "/fs/folder",
        "load-asyn": !0,
    ...
    }, t.setParam = function (t, e) {
        e.mount = t.mount, e.permission = "upload"
    }, t.directory = n.directory, t.directory || (t.directory = {
        mountAlias: "my",
        mount: t.mounts.my,
        name: "我的文档",
        fullName: "我的文档"
    }), t.onChange = function (e) {
        e.id || e.mount !== t.mounts.enterprise ? t.allowed = !0 : t.allowed = !1
    }, t.remove = function (e) {
        t.files.splice(t.files.indexOf(e), 1), 0 === t.files.length && r.dismiss("cancel")
    }, t.ok = function () {
        var e = {
            mountAlias: t.directory.id || t.mountAliases[t.directory.mount],
            parent: t.directory.id,
            path: t.directory.name,
            mount: t.directory.mount
        };
        r.close({files: t.files, uploadContext: e})
    }, t.cancel = function () {
        s.confirm("提示", "是否确定放弃上传?").then(function (t) {
            r.dismiss("cancel")
        }, function () {
        })
    }
}]), app.controller("FavoritesCtrl", ["$rootScope", "$scope", "$http", "$state", "$location", "$modal", "Messager", "FileSystem", function (t, e, n, r, s, i, o, l) {
    function a(t) {
        116 === t.keyCode && (t.preventDefault(), e.load())
    }
    
    t.module = "favorites", e.load = function () {
        e.loading = !0, n.get("/fs/favorites").success(function (t) {
            e.loading = !1, t.success ? e.files = t.data : e.error = t.message
        }).error(function (t, n) {
            e.error = t.message || n, e.toaster.pop("error", t.message || n), e.loading = !1
        })
    }, e.$on("$viewContentLoaded", function () {
        e.load()
    }), e.removeFavorite = function () {
        o.confirm("提示", "确定要删除?").then(function () {
            var n = t.toaster.wait("正在删除...");
            l.removeFavorite(e.selection).then(function (t) {
                n.doSuccess("已删除"), e.load()
            }, function (t) {
                n.doError("删除失败:" + t)
            })
        })
    }, e.isSelectAll = !1, e.selection = [], e.selectAll = function () {
        e.isSelectAll = !e.isSelectAll;
        for (var t = e.files.length, n = 0; n < t; n++) e.select(e.files[n], e.isSelectAll)
    }, e.addSelection = function (t) {
        e.selection.push(t), t.selected = !0, e.selection.length === e.files.length && (e.isSelectAll = !0)
    }, e.removeSelection = function (t) {
        e.selection.splice(e.selection.indexOf(t), 1), t.selected = !1, e.isSelectAll = !1
    }, e.clearSelection = function () {
        e.selection.forEach(function (t) {
            t.selected = !1
        }), e.selection = [], e.isSelectAll = !1
    }, e.select = function (t, n, r) {
        if (r && "block" == e.mode && !r.ctrlKey && e.clearSelection(), r && r.stopPropagation(), !t.selected || n !== t.selected) return !1 === (n = n || !t.selected || !t.selected) ? e.removeSelection(t) : e.addSelection(t)
    }, e.onContainerClick = function () {
        "block" == e.mode && e.clearSelection()
    }, e.remove = function (n) {
        n || 0 === (n = e.selection).length && o.alert("提示", "请选择要删除的文件"), t.remove(n, function () {
            e.reload()
        })
    }, e.reset = function () {
        e.selection = [], e.isSelectAll = !1
    }, $(document).on("keydown", a), e.$on("$destroy", function () {
        $(document).unbind("keydown", a)
    })
}]), app.controller("MessagesCtrl", ["$scope", "$http", "$rootScope", "$location", "$state", function (t, e, n, r, s) {
    t.message || t.$on("$viewContentLoaded", function () {
        t.load()
    }), t.load = function () {
        t.loading = !0, e.get("/message/list").success(function (e) {
            t.loading = !1, e.success ? n.messages = e.data : t.error = "加载消息失败:" + e.message
        }).error(function (e, n) {
            t.loading = !1, t.error = "加载消息失败:" + (e.message || n)
        })
    }
}]), app.controller("ProfileCtrl", ["$scope", "$http", function (t, e) {
    t.save = function () {
        var n = t.toaster.wait("正在保存");
        e.post("/user/updateProfile", {
            avatar: t.user.avatar,
            name: t.user.name,
            mode: t.user.mode,
            sortField: t.user.sortField,
            sortDesc: t.user.sortDesc
        }).success(function (e) {
            e.success ? (t.userSettingForm.$setPristine(), t.userSettingForm.$setUntouched(), n.doSuccess("个人设置已保存")) : n.doError("保存失败：" + e.message)
        }).error(function (t, e) {
            n.doError("保存失败：" + (t.message || e))
        })
    }, t.selectAvatar = function () {
        angular.element("#avatarInput").click()
    }, t.upload = function () {
        var e = angular.element("#avatarInput")[0].files, n = new FormData;
        n.append("avatar", e[0]);
        var r = new XMLHttpRequest;
        r.onload = function (e) {
            if (t.uploading = !1, 200 !== r.status) t.result = {success: !1, message: "上传失败：请稍候再试：" + r.status}; else {
                var n = angular.fromJson(r.response);
                if (t.result = n, n.success) {
                    t.user.avatar = n.avatar;
                    var s = angular.element(".avatar-img");
                    s.attr("src", s.attr("src") + "?_dc=" + (new Date).getTime())
                } else t.message = "上传失败：" + n.message
            }
            t.$apply()
        }, r.onerror = function (e) {
            t.uploading = !1, t.result = {success: !1, message: "上传失败：请稍候再试：" + r.status}
        }, r.open("POST", "/user/upload/avatar"), r.setRequestHeader("X-Requested-With", "XMLHttpRequest"), r.setRequestHeader(crsf.headerName, crsf.token), t.uploading = !0, r.send(n)
    }
}]), app.controller("RecentsCtrl", ["$rootScope", "$scope", "$http", "$state", "$location", "$modal", "Messager", "FileSystem", function (t, e, n, r, s, i, o, l) {
    function a(t) {
        116 === t.keyCode && (t.preventDefault(), e.load())
    }
    
    t.module = "recent", e.load = function () {
        e.loading = !0, n.get("/fs/recent").success(function (t) {
            e.loading = !1, t.success ? e.files = t.data : e.error = t.message
        }).error(function (t, n) {
            e.error = t.message || n, e.toaster.pop("error", t.message || n), e.loading = !1
        })
        ...
        e.load()
    }), e.removeRecent = function () {
        o.confirm("提示", "确定要删除?").then(function () {
            var r = t.toaster.wait("正在删除..."), s = Util.Array.copyProperty(e.selection, "id").join(",");
            n.post("/recent/remove", {hcoId: s}).success(function (t) {
                t.success ? (r.doSuccess("已删除"), e.load()) : r.doError("删除失败:" + t.message)
            }).error(function (t, e) {
                r.doError("删除失败:" + t.message || e)
            })
        })
    }, e.clear = function () {
        o.confirm("提示", "确定要清除所有记录?").then(function () {
            var r = t.toaster.wait("正在清除...");
            n.post("/recent/clear").success(function (t) {
                t.success ? (r.doSuccess("已清除"), e.load()) : r.doError("清除失败:" + t.message)
            }).error(function (t, e) {
                r.doError("清除失败:" + t.message || e)
            })
        })
    }, e.isSelectAll = !1, e.selection = [], e.selectAll = function () {
        e.isSelectAll = !e.isSelectAll;
        ...
        r && "block" == e.mode && !r.ctrlKey && e.clearSelection(), r && ($(".app-ctrl").scrollTop(0), r.stopPropagation()), t.selected && n === t.selected || (!1 === (n = n || !t.selected || !t.selected) ? e.removeSelection(t) : e.addSelection(t))
    }, e.onContainerClick = function () {
        "block" === e.mode && e.clearSelection()
    }, e.remove = function (n) {
        n || 0 === (n = e.selection).length && o.alert("提示", "请选择要删除的文件"), t.remove(n, function () {
...
}]), app.controller("RecycleCtrl", ["$rootScope", "$scope", "$http", "$state", "$location", "$modal", "Messager", function (t, e, n, r, s, i, o) {
    function l(t) {
        116 === t.keyCode && (t.preventDefault(), e.load())
    }
    
    t.module = "recycle", e.load = function () {
        e.reset(), e.loading = !0, n.get("/recycle/list").success(function (t) {
            e.loading = !1, t.success ? e.files = t.data : e.error = t.message
        }).error(function (t, n) {
            e.error = t.message || n, e.toaster.pop("error", t.message || n), e.loading = !1
        })
        ...
        e.load()
    }), e.sortField = "name", e.sortFields = ["type", "name"], e.sortDesc = !1, e.changeSort = function (t) {
        e.sortField === t ? e.sortDesc = !e.sortDesc : (e.sortField = t, e.sortDesc = !0), e.sortFields = ["type", t]
    }, e.changeDesc = function () {
        e.sortDesc = !e.sortDesc
    }, e.revert = function () {
        o.confirm("提示", "确定要恢复?").then(function () {
            var r = t.toaster.wait("正在恢复..."), s = Util.Array.copyProperty(e.selection, "id");
            n.post("/recycle/revert", {documentIds: s}).success(function (t) {
                t.success ? (r.doSuccess("文件已恢复"), e.load()) : r.doError("恢复失败:" + t.message)
            }).error(function (t, e) {
                r.doError("恢复失败:" + (t.message || e))
            })
        })
    }, e.destroy = function () {
        o.confirm("提示", "确定要彻底删除?").then(function () {
            var r = Util.Array.copyProperty(e.selection, "id"), s = t.toaster.wait("正在删除...");
            n.post("/recycle/delete", {documentIds: r}).success(function (t) {
                t.success ? (s.doSuccess("文件已彻底删除"), e.load()) : s.doError("删除失败:" + t.message)
            }).error(function (t, e) {
                s.doError("删除失败:" + (t.message || e))
            })
        })
    }, e.clear = function () {
        o.confirm("提示", "确定要清空回收站?").then(function () {
            var r = t.toaster.wait("正在清空...");
            n.post("/recycle/clear").success(function (t) {
                t.success ? (r.doSuccess("回收站已清空"), e.load()) : r.doError("清空回收站失败:" + t.message)
            }).error(function (t, e) {
                r.doError("清空回收站失败:" + (t.message || e))
            })
        })
    }, e.isSelectAll = !1, e.selection = [], e.selectAll = function () {
        e.isSelectAll = !e.isSelectAll;
        ...
        r && "block" === e.mode && !r.ctrlKey && e.clearSelection(), r && r.stopPropagation(), t.selected && n === t.selected || (!1 === (n = n || !t.selected || !t.selected) ? e.removeSelection(t) : e.addSelection(t))
    }, e.onContainerClick = function () {
        "block" === e.mode && e.clearSelection()
    }, e.remove = function (n) {
        n || 0 === (n = e.selection).length && o.alert("提示", "请选择要删除的文件"), t.remove(n, function () {
        ...
        e.selection = [], e.isSelectAll = !1
    }, $(document).keydown(l), e.$on("$destroy", function () {
        $(document).unbind("keydown", l)
    })
}]), app.controller("SearchCtrl", ["$rootScope", "$scope", "$http", "$state", "$stateParams", "$location", "$modal", "Messager", "FileSystem", function (t, e, n, r, s, i, o, l, a) {
    t.module = "search", e.keyword = s.keyword, t.keyword = e.keyword, e.load = function () {
        e.loading = !0, e.result = [], n.get("/fs/search", {params: e.buildParam()}).success(function (t) {
            e.loading = !1, t.success ? (e.result = t.data, e.pagination.pages = t.data.pages) : e.error = t.message
        }).error(function (t, n) {
            e.error = t.message || n, e.toaster.pop("error", t.message || n), e.loading = !1
        })
    }, e.$on("$viewContentLoaded", function () {
        ...
        e.pagination.page = 1, e.load()
    }, e.loadPage = function (t) {
        e.pagination.page = t, e.load()
    }, e.result = {rows: []}, e.pagination = {size: 20, page: 1, pages: 0}, e.onPageChange = function () {
        e.pagination.page > e.pagination.pages || e.reload()
    }, e.buildParam = function () {
        return {size: e.pagination.size, page: e.pagination.page, keyword: e.keyword}
    }, e.isSelectAll = !1, e.selection = [], e.selectAll = function () {
        e.isSelectAll = !e.isSelectAll;
        for (var t = e.result.records, n = t.length, r = 0; r < n; r++) e.select(t[r], e.isSelectAll)
    }, e.addSelection = function (t) {
        e.selection.push(t), t.selected = !0, e.selection.length === e.result.records.length && (e.isSelectAll = !0)
    }, e.removeSelection = function (t) {
        e.selection.splice(e.selection.indexOf(t), 1), t.selected = !1, e.isSelectAll = !1
    }, e.clearSelection = function () {
        e.selection.forEach(function (t) {
        ...
        n || (n = e.selection), t.download(n)
    }, e.reset = function () {
        e.selection = [], e.isSelectAll = !1
    }, $(document).bind("keydown.hc", function (t) {
        116 === t.keyCode && (t.preventDefault(), e.load())
    }), e.$on("$destroy", function () {
        $(document).unbind("keydown.hc")
    })
}]), app.controller("SelectUserCtrl", ["$rootScope", "$scope", "$http", "$modal", "$modalInstance", "options", function (t, e, n, r, s, i) {
    e.loading = !1, e.tree = {}, e.onTreeInit = function () {
        var t = e.tree.getRoot();
        e.tree.select(t)
    }, e.multiple = i.multiple, e.clear = function () {
        e.searching = !1, e.condition = null
    }, e.selected = [], e.selectedIds = [], e.select = function (t) {
        e.selectedIds.indexOf(t.id) >= 0 || (!1 === e.multiple && (e.selected = [], e.selectedIds = []), e.selected.push(t), e.selectedIds.push(t.id))
    }, e.remove = function (t) {
        e.selectedIds.indexOf(t.id) < 0 || (e.selected.splice(e.selected.indexOf(t), 1), e.selectedIds.splice(e.selectedIds.indexOf(t.id), 1))
    }, e.search = function () {
        e.condition && (e.searching = !0, e.loading = !0, n.get("/user/search", {params: {condition: e.condition}}).success(function (t, n, r, s) {
            e.loading = !1, t.success ? e.result = t.data : e.toaster.pop("error", "提示", t.message)
        }).error(function (t, n) {
            e.loading = !1, e.toaster.pop("error", "提示", "加载失败：" + n)
        }))
    }, e.ok = function () {
        s.close(e.selected)
    }, e.loadUser = function (t) {
        e.loading = !0, n.get("/user/list", {params: {departmentId: t.id}}).success(function (t, n, r, s) {
            e.loading = !1, console.log(t), t.success ? e.users = t.data : e.toaster.pop("error", t.message)
        }).error(function (t, n) {
            e.loading = !1, e.toaster.pop("error", "加载失败：" + n)
        })
    }, e.close = function () {
...
}]), app.controller("ShareCtrl", ["$rootScope", "$scope", "$http", "$state", "$location", "$modal", "Messager", "FileSystem", function (t, e, n, r, s, i, o, l) {
    t.module = "share", e.load = function () {
    }, e.$on("$viewContentLoaded", function () {
        e.error = "非常抱歉，社区版暂不支持该功能"
    })
}]), app.factory("Dropbox", ["$rootScope", "FileSystem", "$modal", function (t, e, n) {
    var r = {};
    return r.init = function (t) {
        ...
        var r = document.getElementById("dropbox"), s = !1;
        document.addEventListener("mousedown", function () {
            s = !0
        }), document.addEventListener("mouseup", function () {
            s = !1
        }), document.addEventListener("dragenter", function (t) {
            s || (r.innerText = "文件拖放到此处上传", r.style.display = "block")
        }, !1), r.addEventListener("dragenter", function (t) {
            e(t)
        }), r.addEventListener("dragover", function (t) {
            e(t)
        }), r.addEventListener("dragleave", function (t) {
            e(t), r.style.display = "none"
        }), r.addEventListener("drop", function (s) {
            e(s), r.style.display = "none";
            var i = s.dataTransfer.files;
            i && i.length > 0 && n.open({
                ...
                for (var n = e.files, r = 0; r < n.length; r++) {
                    var s = n[r];
                    s.uploadContext = e.uploadContext, t.webUploader.addFiles(s)
                }
            }, function () {
            })
        })
    }, r
}]), app.factory("FileOperation", ["$rootScope", "FileSystem", "$modal", "Messager", "$state", function (t, e, n, r, s) {
    var i = {};
    return i.init = function (i) {
        i.openParent = function (t, e) {
            s.go("directory", {directory: -1 !== t.parent ? t.parent : i.getMountAlias(t.mount)}), e.stopPropagation()
        }, t.open = function (e, n) {
            "directory" === e.type ? s.go("directory", {directory: e.id}) : (t.fid = e.id, $location.path("/index/document/" + e.id), n && n.stopPropagation())
        }, t.closeFile = function (e) {
            t.fileModal && (t.fileModal.dismiss("cancel"), t.fileModal = null)
        }, t.share = function (t) {
            r.alert("提示", "非常抱歉，社区版暂不支持该功能")
        }, t.sendFile = function (t) {
            angular.isArray(t) || (t = [t]), n.open({
            ...
            r.result.then(function (t) {
                !0 === t && i.$broadcast("$currentFolderReload", t.file)
            }, function () {
            }), t.fileModal = r, t.fid = null
        }, t.remove = function (t, n, s) {
            r.confirm("提示", "文件删除之后可以在回收站内还原，是否确定删除选中的文件?").then(function () {
                var r = i.toaster.wait({title: "正在删除文件..."});
                e.remove(t).then(function (t) {
                    r.doSuccess("删除文件成功"), angular.isFunction(n) && n(t)
                }, function (t) {
                    r.doError("删除文件失败:" + t), angular.isFunction(s) && s(t)
                })
            })
        }, t.lock = function (t, n, s) {
            r.confirm("提示", "锁定文件后只能本人进行解锁，确定要锁定?").then(function () {
                var r = i.toaster.wait({title: "正在锁定..."});
                e.lock(t).then(function (t) {
                    r.doSuccess("文件已锁定"), angular.isFunction(n) && n(t)
                }, function (t) {
                    r.doError("锁定文件失败:" + t), angular.isFunction(s) && s(t)
                })
            })
        }, t.unlock = function (t, n, s) {
            r.confirm("提示", "确定要解锁文件?").then(function () {
                var r = i.toaster.wait({title: "正在解锁..."});
                e.unlock(t).then(function (t) {
                    r.doSuccess("文件已解锁"), angular.isFunction(n) && n(t)
                }, function (t) {
                    r.doError("解锁文件失败:" + t), angular.isFunction(s) && s(t)
                })
            })
        }, t.setTag = function (t, e, r) {
            n.open({
        ...
        }, t.rename = function (t, n, s) {
            r.prompt("重命名", "", t.name).then(function (r) {
                var o = i.toaster.wait({title: "正在重命名..."});
                e.rename(t.id, r).then(function (t) {
                    o.doSuccess({title: "重命名成功"}), angular.isFunction(n) && n(t, r)
                }, function (t) {
                    o.doError({title: "重命名失败", body: t}), angular.isFunction(s) && s(t)
                })
            })
        }, t.move = function (t, r, s, o) {
            n.open({
                ...
                resolve: {
                    data: function () {
                        return {files: t, mount: r}
                    }
                }
            }).result.then(function (n) {
                var r = i.toaster.wait({title: "正在移动..."});
                e.move(t, n.id, n.mount).then(function (t) {
                    r.doSuccess({title: "移动成功"}), angular.isFunction(s) && s(t)
                }, function (t) {
                    r.doError({title: "移动失败", body: t}), angular.isFunction(o) && o(t)
                })
            })
        }, t.copy = function (t, r, s) {
            n.open({
                ...
                var r = i.toaster.wait({title: "正在复制..."});
                e.copy(t, n.id, n.mount).then(function (t) {
                    r.doSuccess({title: "复制成功"}), angular.isFunction(success) && success(t)
                }, function (t) {
                    r.doError({title: "复制失败", body: t}), angular.isFunction(s) && s(t)
                })
            })
        }, t.updateDescription = function (t, n, r) {
            var s = i.toaster.wait({title: "正在修改备注..."});
            e.updateDescription(t).then(function (t) {
                s.doSuccess({title: "修改成功"}), angular.isFunction(n) && n(t, name)
            }, function (t) {
                s.doError({title: "修改失败", body: t}), angular.isFunction(r) && r(t)
            })
        }, t.download = function (t) {
            i.downloadURL = "/fs/download?id=" + t.id
        }, t.uploadNewVersion = function (t) {
            r.alert("提示", "非常抱歉，社区版不支持文档多版本")
        }
    }, i
}]), app.factory("FileSystem", ["$http", "$q", function (t, e) {
    var n = {};
    return n.isPrivate = function (t) {
        return "private" == t.accessiblity
    }, n.isDirectory = function (t) {
        return !!t && "directory" == t.type
    }, n.isRoot = function (t) {
        return !t.root || t.root == t.id
    }, n.download = function (n) {
        var r = e.defer(), s = n.id;
        return angular.isArray(n) && (s = Util.Array.copyProperty(n, "id").join(",")), t.get("/fs/download", {params: {id: s}}).success(function () {
            r.resolve()
        }), r.promise
    }, n.createDirectory = function (n) {
        var r = e.defer();
        return t.post("/fs/createDirectory", n).success(function (t) {
            t.success ? r.resolve(t) : r.reject(t.message)
        }).error(function (t, e) {
            r.reject(e)
        }), r.promise
    }, n.load = function (n, r) {
        var s = e.defer();
        return t.get("/fs/directory/", {params: {id: r, mount: n}}).success(function (t) {
            t.success ? s.resolve(t.data) : s.reject(t.message)
        }).error(function (t, e) {
            s.reject(e)
        }), s.promise
    }, n.addFavorite = function (n) {
        var r = e.defer();
        return t.post("/favorite/add", {documentId: n.id}).success(function (t) {
            t.success ? r.resolve(t) : r.reject(t.message)
        }).error(function (t, e) {
            r.reject(e)
        }), r.promise
    }, n.removeFavorite = function (n) {
        var r = e.defer(), s = n.id;
        return s = angular.isArray(n) ? Util.Array.copyProperty(n, "id") : [s], t.post("/favorite/remove", {documentIds: s}).success(function (t) {
            t.success ? r.resolve(t) : r.reject(t.message)
        }).error(function (t, e) {
            r.reject(e)
        }), r.promise
        ...
        var r = e.defer();
        return t.post("/fs/tag", {id: n.id, tags: n.tags}).success(function (t) {
            t.success ? r.resolve(t) : r.reject(t.message)
        }).error(function (t, e) {
            r.reject(e)
        }), r.promise
    }, n.updateDescription = function (n) {
        var r = e.defer();
        return t.post("/fs/updateDescription", {id: n.id, description: n.description}).success(function (t) {
            t.success ? r.resolve(t) : r.reject(t.message)
        }).error(function (t, e) {
            r.reject(e)
        }), r.promise
    }, n.copy = function (n, r, s) {
        var i = e.defer();
        return n = angular.isArray(n) ? Util.Array.copyProperty(n, "id") : [n.id], t.post("/fs/copy", {
            documentIds: n,
            parent: r,
            mount: s
        }).success(function (t) {
            t.success ? i.resolve(t) : i.reject(t.message)
        }).error(function (t, e) {
            i.reject(e)
        }), i.promise
    }, n.move = function (n, r, s) {
        var i = e.defer();
        return n = angular.isArray(n) ? Util.Array.copyProperty(n, "id") : [n.id], t.post("/fs/move", {
            documentIds: n,
            parent: r,
            mount: s
        }).success(function (t) {
        ...
        var r = e.defer(), s = n.id;
        return s = angular.isArray(n) ? Util.Array.copyProperty(n, "id") : [s], t.post("/fs/remove", {documentIds: s}).success(function (t) {
            t.success ? r.resolve(t) : r.reject(t.message)
        }).error(function (t, e) {
            r.reject(t.message || e)
        }), r.promise
    }, n.lock = function (n) {
        var r = e.defer();
        return t.post("/fs/lock", {documentIds: [n.id]}).success(function (t) {
            t.success ? r.resolve(t) : r.reject(t.message)
        }).error(function (t, e) {
            r.reject(t.message || e)
        }), r.promise
        ...
        var r = e.defer();
        return t.post("/fs/unlock", {documentIds: [n.id]}).success(function (t) {
            t.success ? r.resolve(t) : r.reject(t.message)
        }).error(function (t, e) {
            r.reject(t.message || e)
        }), r.promise
    }, n.rename = function (n, r) {
        var s = e.defer();
        return t.post("/fs/rename", {id: n, name: r}).success(function (t) {
            t.success ? s.resolve(t) : s.reject(t.message)
        }).error(function (t, e) {
            s.reject(e)
        }), s.promise
    }, n.verify = function (n) {
        var r = e.defer();
        return t.post("/fs/upload/verify", {
            md5: n.md5,
            name: n.name,
            path: n.path,
            ext: n.ext,
            ...
            t.success ? r.resolve(t) : r.reject(t)
        }).error(function (t, e) {
            r.reject(t)
        }), r.promise
    }, n
}]), app.factory("SocketIO", ["$rootScope", "FileSystem", "$modal", function (t, e, n) {
    var r = {};
    return r.init = function (t) {
        if (window.WebSocket) {
            var e = new WebSocket("ws://localhost:8080/socket.io");
            e.onopen = function () {
            }, e.onclose = function () {
            }, e.onerror = function () {
            }
...
}]), app.factory("WebUploader", ["$rootScope", "FileSystem", function (t, e) {
    var n = {};
    return n.init = function (n) {
        function r() {
            n.uploadStats = n.webUploader.getStats()
        }
        
        if (t.onUploadFileChange = function () {
            var t = document.getElementById("uploadFile").files;
            if (t) for (var e = t.length, r = 0; r < e; r++) {
                var s = t[r], i = WebUploader.guid();
                n.webUploader.addFile(s, i)
            }
        }, window.WebUploader) {
            WebUploader.Uploader.register({
                ...
                beforeSendFile: function (t) {
                    var r = WebUploader.Deferred();
                    return t.setStatus("md5", "正在校验文件"), n.webUploader.md5File(t, 0, 10485760).progress(function (e) {
                        t.md5Percentage = e
                    }).then(function (n) {
                        t.md5 = n, e.verify(t).then(function (e) {
                            e.success ? r.resolve() : t.setStatus("error", e.message || "文件校验失败")
                        }, function (e) {
                            t.setStatus("error", e.message), r.reject()
                        })
                    }), r.promise()
                }
            });
